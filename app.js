...
async function getKey(password) {
    ...
    return window.crypto.subtle.deriveKey(
        {
            name: "PBKDF2",
            salt: enc.encode("elven-salt-12345"),
            iterations: 100000,
            hash: "SHA-256"
        },
        keyMaterial,
...
document.getElementById('btn-pdf').addEventListener('click', async () => {
    ...
    doc.setFillColor(236, 220, 180);
    doc.rect(0, 0, 210, 297, "F");

    doc.setDrawColor(74, 59, 34);
...
document.getElementById('editor').addEventListener('copy', async (e) => {
    e.preventDefault();

    const htmlContent = document.getElementById('editor').innerHTML;
    if (!htmlContent.trim()) return;

    const password = prompt("Вы копируете эльфийский текст. Введите пароль, чтобы зашифровать его в буфер обмена:");
    if (!password) return alert("Копирование отменено. Без пароля текст нельзя скопировать как шифр.");
    ...
    try {
        ...
        const secureElvenString = `---BEGIN ELVEN SCROLL---\n${base64String}\n---END ELVEN SCROLL---`;

        if (e.clipboardData) {
            e.clipboardData.setData('text/plain', secureElvenString);
            alert("В буфер обмена скопирован надежный сквозной шифр! Смена шрифта в Блокноте его не раскроет.");
        }
    } catch (err) {
        alert("Не удалось зашифровать текст при копировании: " + err.message);
    }
});
