...
// Функция для форматирования текста (жирный, курсив и т.д.)
function format(command) {
    document.execCommand(command, false, null);
    document.getElementById('editor').focus();
}

// Вспомогательные функции для работы с криптографией (Генерация ключа из пароля)
...
// Хэндлер кнопки сохранения
document.getElementById('btn-save').addEventListener('click', async () => {
    const htmlContent = document.getElementById('editor').innerHTML;
    if (!htmlContent.trim()) return alert("Document is empty");

    const password = prompt("Enter the key:");
    if (!password) return;
    ...
    try {
        ...
        // НАШ СОБСТВЕННЫЙ ФОРМАТ: 
        // Символы Base64 (A-Z, a-z, 0-9, +, /) можно превратить в эльфийские маркеры.
        // Для простоты и совместимости со шрифтом Tengwar, запишем его с уникальным заголовком
        ...
        a.href = url;
        a.download = "scroll.elven"; // Наше собственное расширение
        a.click();
...
fileInput.addEventListener('change', async (e) => {
    ...
    reader.onload = async (event) => {
        ...
        // Проверяем сигнатуру нашего формата
        if (!content.includes('---BEGIN ELVEN SCROLL---')) {
            return alert("Not right format");
        }
        ...
        try {
            // Извлекаем чистую строку Base64
            ...
            // Вытаскиваем IV (первые 12 байт) и зашифрованные данные
            ...
            // Загружаем обратно в Ворд-редактор с сохранением всех стилей!
            document.getElementById('editor').innerHTML = resultHTML;
            alert("Document uploaded!");

        } catch (error) {
            alert("Corrupted file or invalid key!");
        }
    };
...
document.getElementById('btn-pdf').addEventListener('click', async () => {
    // Получаем чистый текст из редактора (jsPDF лучше работает с текстом построчно)
    const editor = document.getElementById('editor');
    const textContent = editor.innerText;

    if (!textContent.trim()) {
        return alert("Редактор пуст! Нечего переносить на свиток.");
    }

    // Инициализируем jsPDF (в глобальной области видимости через UMD)
    ...
    // === КРИТИЧЕСКИЙ ШАГ: ВНЕДРЕНИЕ ШРИФТА ===
    // Вставь сюда строку Base64, полученную из твоего файла tngan.ttf
    ...
    try {
        // Добавляем кастомный шрифт в виртуальную файловую систему jsPDF
        ...
        return alert("Не удалось загрузить эльфийский шрифт для PDF. Проверьте строку Base64.");
    }

    // === ДИЗАЙН: ОФОРМЛЕНИЕ ПЕРГАМЕНТА ===
    // Заливаем всю страницу красивым светло-бежевым «пергаментным» цветом
    doc.setFillColor(244, 236, 216); // #f4ecd8 в формате RGB
    doc.rect(0, 0, 210, 297, "F");  // Размеры листа А4 в мм

    // Рисуем изящную двойную коричневую рамку по краям свитка
    doc.setDrawColor(92, 74, 51);    // Коричневый цвет границ
    doc.setLineWidth(0.5);
    doc.rect(8, 8, 194, 281);       // Внешняя рамка
    doc.rect(10, 10, 190, 277);     // Внутренняя рамка

    // === ДОБАВЛЕНИЕ ТЕКСТА ===
    doc.setTextColor(44, 26, 4);     // Глубокий темно-коричневый цвет текста
    doc.setFontSize(16);             // Размер эльфийских букв

    // Автоматический перенос строк, чтобы текст не вылезал за рамки листа (отступы по 20мм)
    const splitText = doc.splitTextToSize(textContent, 170);
    
    // Печатаем текст на странице с координатами X=20, Y=25
    doc.text(splitText, 20, 25);

    // Смена темы интерфейса на эльфийскую для атмосферности при скачивании
    document.documentElement.setAttribute('data-theme', 'elven');
